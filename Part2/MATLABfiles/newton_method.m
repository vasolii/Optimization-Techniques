function k_final=newton_method(x_start,y_start,option_gamma)
...
while norm(f_gr(x_y_val(1,k),x_y_val(2,k)))>=const_fin
    d(:,k)=-(inv(f_gr_2(x_y_val(1,k),x_y_val(2,k)))*f_gr(x_y_val(1,k),x_y_val(2,k)));
...
sgtitle("\bf Newton Method");

filename = sprintf('newton_%d_%d.jpg', x_start, option_gamma);
saveas(gcf,filename);
