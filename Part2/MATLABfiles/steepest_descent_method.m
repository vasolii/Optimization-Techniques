function k_final=steepest_descent_method(x_start,y_start,option_gamma)
...
while norm(f_gr(x_y_val(1,k),x_y_val(2,k)))>=const_fin
    d(:,k)=-f_gr(x_y_val(1,k),x_y_val(2,k));
    if option_gamma==1

        gamma(k)=0.5; %in case of const gamma
...
filename = sprintf('steep_%d_%d.jpg', x_start, option_gamma);
saveas(gcf,filename);
